...
use AssoMaker\PHPMBundle\Entity\Orga;
...
class OrgaController extends Controller
{
    ...
    public function affectationAction()
    {
     if (false === $this->get('security.context')->isGranted('ROLE_ADMIN')) {
        ...
        $entities = $em->getRepository('AssoMakerPHPMBundle:Orga')->findAll();
    ...
    public function indexAction($statut, $confiance)
    {
        ...
        $confiances =$em
        ->createQuery("SELECT c FROM AssoMakerPHPMBundle:Confiance c")
        ->getResult();
        
        
        $orgasDQL = "SELECT o,e,d FROM AssoMakerPHPMBundle:Orga o JOIN o.equipe e JOIN e.confiance c LEFT JOIN o.disponibilites d WHERE o.statut = $statut ";
    ...
    public function registerSoftAction($confianceCode,$equipeId){
    	...
    	$equipe = $em->getRepository('AssoMakerPHPMBundle:Equipe')->find($equipeId);
    	...
    	if ($this->get('request')->getMethod() == 'POST') {
    		...
    		if ($form->isValid()) {
    			...
    			->setBody($this->renderView('AssoMakerPHPMBundle:Orga:emailConfirmationSoft.html.twig', array('orga' => $entity)), 'text/html')
    			;
     ...
     * @Method("post")
     * @Template("AssoMakerPHPMBundle:Orga:register.html.twig")
     */
 ...
 /**
     ...
     * @Method("post")
     * @Template("AssoMakerPHPMBundle:Orga:new.html.twig")
     */
    ...
    public function editAction($id)
    {
        ...
        $em = $this->getDoctrine()->getEntityManager();
        $config = $e=$this->get('config.extension');
        $entity = $em->getRepository('AssoMakerPHPMBundle:Orga')->find($id);
     ...
     * @Method("post")
     * @Template("AssoMakerPHPMBundle:Orga:edit.html.twig")
     */
    ...
    */
	...
	public function inputDisposAction($id)
	{
	    $request = $this->getRequest();
	    $em = $this->getDoctrine()->getEntityManager();
	    $orga =  $em->getRepository('AssoMakerPHPMBundle:Orga')->find($id);
	...
	*/
	public function queryJsonAction()
	{
		...
		if ($creneau_id === '') {
			$entities = $em->getRepository('AssoMakerPHPMBundle:Orga')->getOrgasWithCriteria($annee_permis, $maxDateNaissance, $plage_id, $niveau_confiance, $equipe_id);
		} else {
			$entities = $em->getRepository('AssoMakerPHPMBundle:Orga')->getOrgasCompatibleWithCreneau($creneau_id);			
		}
	 ...
	 * @Template("AssoMakerPHPMBundle:Orga:printPlanning.html.twig")
	 */
	public function printAction($orgaid)
	{
        ...
        $em = $this->getDoctrine()->getEntityManager();
        $orgas = $em->getRepository('AssoMakerPHPMBundle:Orga')-> getPlanning($orgaid,'all',$debut,$fin);
...
//         			$creneau['fin']=  new \DateTime();
	...
	public function planningsAction()
	{
		...
		$orgai = $em->getRepository('AssoMakerPHPMBundle:Orga')->find($orgaid);
		
		$equipei = $em->getRepository('AssoMakerPHPMBundle:Orga')->find($equipeid);
		...
		$em = $this->getDoctrine()->getEntityManager();
		$orgas = $em->getRepository('AssoMakerPHPMBundle:Orga')-> getPlanning($orgaid,$equipeid,$debut,$fin);
		foreach ($orgas as &$orga){
			...
			if($action=='mail'){
				...
				->setReplyTo('orga@24heures.org')
				->setTo($orga['email'])
				->setBody($this->renderView('AssoMakerPHPMBundle:Orga:emailPlanning.html.twig', array('orga' => $orga)), 'text/html')
				;
				$this->get('mailer')->send($message);
		...
		if($action=='show'||$action=='mail'){	
			...
			return $this->render('AssoMakerPHPMBundle:Orga:showPlanning.html.twig', array('orgas' => $orgas,'debut'=>$debut, 'fin'=>$fin,'printPlanningForm'=>$printPlanningForm->createView()));				
		}
		...
		return $this->render('AssoMakerPHPMBundle:Orga:printPlanning.html.twig', array('orgas' => $orgas,'debut'=>$debut, 'fin'=>$fin));	
	...
	public function iCalAction($orgaid)
	{
		...
		$em = $this->getDoctrine()->getEntityManager();
		$orgas = $em->getRepository('AssoMakerPHPMBundle:Orga')-> getPlanning($orgaid,null,$debut,$fin);
		 
		 
		$content = $this->renderView('AssoMakerPHPMBundle:Orga:planning.ics.twig', array('orga' => $orgas[0],'debut'=>$debut, 'fin'=>$fin));
	...
	public function charismeAction()
	{
		...
		$stats = $em->getRepository('AssoMakerPHPMBundle:Orga')->getStats($user);
	
	    $data = $em->getRepository('AssoMakerPHPMBundle:Orga')->findAll();
	
	    $departs = $em
	    ->createQuery("SELECT o.departement, sum(d.pointsCharisme) as pc FROM AssoMakerPHPMBundle:Orga o LEFT JOIN o.disponibilitesInscription d  WHERE o.statut >=0 GROUP BY o.departement ORDER BY pc DESC  ")
	    ->getResult();
	    
	
	    $orgas = $em
	    ->createQuery("SELECT o, sum(d.pointsCharisme) as pc FROM AssoMakerPHPMBundle:Orga o JOIN o.disponibilitesInscription d  WHERE o.statut >=0 GROUP BY o.id ORDER BY pc DESC")
	    ->getResult();
	    
	    $equipes = $em
	    ->createQuery("SELECT e, sum(d.pointsCharisme) as pc FROM AssoMakerPHPMBundle:Equipe e LEFT JOIN e.orgas o LEFT JOIN o.disponibilitesInscription d  WHERE o.statut >=0 GROUP BY e ORDER BY pc DESC  ")
	    ->getResult();
...
// // 		if($id = "all"){
// // 			$orgas = $em->createQuery("SELECT o FROM AssoMakerPHPMBundle:Orga o")->getResult();
			
// // 		}else
// 		{
// 		$orgas = $em->getRepository('AssoMakerPHPMBundle:Orga')->find($id);
...
// 	}
	...
	*/
	public function exportAction()
	{
		...
		$orgas = $em
		->createQuery("SELECT o,d,e FROM AssoMakerPHPMBundle:Orga o JOIN o.disponibilites d JOIN o.equipe e WHERE o.statut >=0")
		->getArrayResult();
