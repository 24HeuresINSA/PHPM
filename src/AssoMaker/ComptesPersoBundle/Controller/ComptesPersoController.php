...
class ComptesPersoController extends Controller
{
	...
	public function comptesPersoAction(Request $request)
	{
		...
		$entities =$em->createQuery("SELECT o FROM AssoMakerBaseBundle:Orga o")->getResult();
		...
		foreach ($entities as $entity){
			...
			$orga['balance'] = $em->getRepository('AssoMakerComptesPersoBundle:Transaction')->getOrgaBalance($entity->getId());
			$orgas[$entity->getId()]=$orga;
			
			$consosBuilder->add((string) $orga['id'],'number',array('required'=> false));
		...
		return array('prixConsoStandard'=>$prixConso,'orgas' => $orgas,'totalBalance'=>$totalBalance,'form' => $form->createView()
				);
	}
